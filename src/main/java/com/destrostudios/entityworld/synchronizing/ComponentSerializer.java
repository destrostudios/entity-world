...
import com.destrostudios.entityworld.bit.BitUtil;
...
public class ComponentSerializer{
    ...
    public static void writeClass(BitOutputStream outputStream, Class<?> componentClass) {
        ...
        int neededBits = BitUtil.getNeededBitsCount(componentManager.getCount());
    ...
    private static Object readObject(BitInputStream inputStream, Class<?> type, FieldSerializer<?> fieldSerializer) throws IOException {
        if (type.isArray()) {
            ...
            try {
                Class<? extends Collection> collectionClass = (Class<? extends Collection<?>>) readClass(inputStream);
    ...
    private static LinkedList<Field> getAllSerializedFields(Class<?> type) {
        return addAllSerializedFields(type, new LinkedList<>());
    }

...
public class BitInputStream {
    ...
    public int readBits(int count) throws IOException {
        ...
        while (remainingCount > 0) {
            ...
            // Copy the smaller of the two: remaining bits or bits left in lastByte.
            int bitsToCopy = Math.min(bits, remainingCount);
 
            // How much do we have to shift the read byte to just get the high bits we want?
            int sourceShift = bits - bitsToCopy;
 
            // And how much do we have to shift those bits to graft them onto our result?
            ...
            // Now we need to mask off the bits we just copied from lastByte. Just keep the bits that are left.
